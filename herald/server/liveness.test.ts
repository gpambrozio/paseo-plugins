...
function entry(agentId: string): AttentionEntry {
  return {
    agentId,
    workspaceId: "w1",
    workspaceTitle: null,
...
type Snapshot = { status: string; archivedAt?: string | null } | null | Error;

function fakePaseo(snapshots: Record<string, Snapshot>) {
...
describe("Liveness.visible", () => {
  it("shows live agents, hides closed sessions, and removes archived or missing agents", async () => {
    ...
    const visible = await new Liveness(() => 0).visible(store, paseo);
    warn.mockRestore();
  ...
  it("asks the daemon about an agent at most once per TTL", async () => {
    ...
    let now = 1_000;
    const { paseo, refresh } = fakePaseo({ a1: { status: "idle" } });
